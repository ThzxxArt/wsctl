...
def test_rotation_keeps_append_descriptor_usable(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert path.with_name("app.log.1").read_text() == "A" * 100
        # ...and the *same* descriptor continues into the fresh file.
        os.write(fd, b"B" * 10)
        assert path.read_text() == "B" * 10
    finally:
        os.close(fd)
...
def test_backups_shift_and_are_capped(tmp_path: Path) -> None:
    ...
    assert path.with_name("app.log.1").read_text() == "3" * 30
    assert path.with_name("app.log.2").read_text() == "2" * 30
